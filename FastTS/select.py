...
def reject_outliers(prev_dir, out_dir, thr_num=5, thr_len=60, thr_std=0.03,
    ...
    for dire in prev_dirs:
        ...
        for i in range(len(disp_files)):
            ...
            # gradient
            mask = (np.gradient(results[i]) > 0) & mask
        ...
        mask = (sstd / smean) < thr_std
        mask = mask & (np.sum(~np.isnan(selected), axis=0) > thr_num)
